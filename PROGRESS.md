...
- `ParseResult<T> = { value: T, warnings: Warning[] }` mit
  `Warning = { code, datei, zeile?, meldung }` — **Parse-Warnungen** (D1, D3, D4, D6,
  D7, D9, D10) entstehen beim Parsen einzelner Dateien; **Validate-Funde** (D2, D5,
  D8, D11, D12) prüfen Querkonsistenz über Dateien hinweg. `docs_validate` = Aggregator
  (Konsistenzregeln + eingesammelte Parse-Warnungen).
- Item-Blöcke tragen ab 1.2 `span: { start, end }` (Zeilenbereich) und `roh`
...
- **1.2 BACKLOG-Parser:** Sektionen + Prioritäten erkennen; Item-Blöcke zu
  `{id, titel, prio, sektion, offen, ort, text, span, roh}`; `[ ]`/`[x]`; Erledigt-Index-Einzeiler
