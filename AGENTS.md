...
10. **Sprache: Englisch für Methoden-Doku + Tool-Fläche, Toleranzschicht für Projektdateien (bindend, 09/2026; erweitert I1/14.1, geschärft I3/16.3):** **Englisch** ist die verbindliche Sprache der **Methoden-Doku und der gesamten Tool-Fläche** — PLAYBOOK/LESSONS (seit 16.1/16.2 vollständig EN, DE-Snapshots in `docs/archive/*`), Sektions-Headings, Feld-Labels (Location, Acceptance, Verification, Scope), Erledigt-Index (Done Index), Statuswörter, Tool-Beschreibungen, CLI-Help, Warning-Messages, Templates, SKILL. **Inhaltstexte** (Item-Bodies, Notizen, Nutzertexte) bleiben bewusst sprachfrei — jeder schreibt sie in seiner Sprache. **Toleranzschicht für Projektdateien (Schärfung 16.3):** Die EN-Pflicht gilt nur für die Methoden-Doku — Projekt-Handbücher und Projekt-Doku (AGENTS.md-Prosa wie diese, READMEs, projektspezifische Dateien) dürfen in der Projektsprache bleiben; die Union-Matching-Toleranz (liest DE+EN still für alle Rollen-Synonyme) bleibt für alle offenen Dateien bestehen. Begründung: API/Struktur-Code und Prosa bleiben deckungsgleich; Nicht-Deutschsprachige können die Methode lesen. **Übergangs-Toleranz (Entscheidung I1/14.6):** Legacy-DE-Labels in offenen Dateien werden stillschweigend gelesen (Union-Matching in `profile.ts` absorbiert DE+EN) — **keine** `STRUCT_LOCALE`-Warnung im `docs_validate`. Die Toleranz ist Teil der Parser-Semantik, keine Beobachtungspflicht: wer ein Item sowieso anfasst, dreht die Labels beim natürlichen Edit auf EN (Datum 09/2026, gleitend). Append-only-Archive werden nie beanstandet (W2). Verbleibende deutsche Begriffe leben nur in historischer Prosa; die Migration (14.2, 16.1/16.2) hat die Methoden-Fläche zu kanonischen EN-Begriffen gemacht oder dokumentiert die Toleranz.
